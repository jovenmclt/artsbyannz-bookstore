<template>
    ...
    <div id="completedCustomer" class="table-responsive rounded border-top border-secondary" style="max-height: 900px; overflow: auto; scrollbar-width:none;">
            ...
            <tbody>
                <template v-for="(customer, index) in get_computedCustomer" :key="customer.id">
                    <tr>
                        ...
                        <td> <img class="bg-transparent toggle-icon" width="25" height="25" src="https://img.icons8.com/office/40/circled-chevron-down.png" alt="toggle"
                    ...
                    <tr v-if="isExporting || collapsedRow === index">
                        <td colspan="7">
                            <table class="table table-bordered table-dark" >
                                ...
                                <tbody>
                                    <tr v-for="(completed, subIndex) in get_completedItems.filter(p => p.clientEmail === customer.clientEmail)" :key="subIndex">
                                        <td class="fw-light" style="font-size: 13px;">{{ completed.payment_id }}</td>
                                        <td><inertiaLink :href="`/viewItem/${completed.premade_id}`"><img :src="`/storage/${completed.itemFile}`" alt="" width="50" class="rounded"></inertiaLink></td>
                                        <td class="fw-light" style="font-size: 13px;" v-html="completed.itemTitle.replace(/(.{30})/g, '$1<br>')"></td>
                                        <td class="fw-light" style="font-size: 13px;">{{ completed.quantity }}</td>
                                        <td class="fw-light" style="font-size: 10px;"><span class="px-2 py-2 border border-success rounded" style="background-color: rgba(134, 242, 114, 0.2);">${{ completed.amount }}</span></td>
                                        <td class="fw-light" style="font-size: 13px;">{{ completed.paymentMethod }}</td>
                                        <td class="fw-light" style="font-size: 13px;">{{ completed.country }}</td>
                                        <td class="fw-light" style="font-size: 13px;">{{ completed.postalCode }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
    ...
    <div class="py-3 px-4 bg-dark rounded-2">
        <div class="text-start bg-transparent">
            <span class="fw-semibold text-primary bg-transparent" style="font-size: 13px;">Showing 1 to {{ get_completedItems.length }} of {{get_completedItems.length}} result.</span>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        name: 'pending',
        props: { get_completedCutomers: Object, get_completedItems: Array },
        ...
        methods: {
            sortID() {
                ...
                this.get_completedCutomers.sort((a, b) => {
                    if (this.rsortIDicon) {
            ...
            exportData() {
                ...
                const element = document.getElementById('completedCustomer');
                element.classList.add('pdf-black-background');

                const options = {
                    margin: 0.5,
                    filename: 'artsbyannz-completedcustomer.pdf',
        ...
        computed:{
            get_computedCustomer(){
                return this.get_completedCutomers.filter(getCustomer =>
